...
fun _ pandnp => (pandnp.right pandnp.left)
...
-- B. Formally prove it using example syntax
example : resolution :=
fun _ _ P a_or_p np_or_b =>
  (
    match (Classical.em P) with
    | Or.inl p=> match np_or_b with
      | Or.inl np => False.elim (np p)
      | Or.inr b => Or.inr b
    | Or.inr np => match a_or_p with
      | Or.inl a => Or.inl a
      | Or.inr p => False.elim (np p)
  )

-- The resolution rule says that if (A ∨ P) and (¬P ∨ B), we can conclude (A ∨ B).
-- Case 1: P is true
-- - If A is true, then A ∨ B is true.
-- - If P is true, then ¬P is false, so B must be true from (¬P ∨ B), making A ∨ B true.

-- Case 2: P is false
-- - If A is true, then A ∨ B is true.
-- - If P were true, it would contradict ¬P, so this case is impossible.
